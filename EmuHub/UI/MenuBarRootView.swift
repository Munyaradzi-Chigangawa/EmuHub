...
private struct CheckForUpdatesPage: View {
    @EnvironmentObject var state: AppState
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("EmuHub checks the latest GitHub Release generated by your workflow and compares it to your current app version.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
                ...
                if let result = state.updateCheckResult {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Current: \(result.currentVersion)")
                            .font(.subheadline)
